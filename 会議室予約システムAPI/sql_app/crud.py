...
def get_users(db:Session,skip:int=0,limit:int=100):
    # query 引数で処理対象のDBを設定 skip 上から何件スキップしますか　limit　上限でもってくる件数
...
def get_search_room(db:Session,room_name:str):
    return db.query(models.Room).filter(room_name == room_name).first()
